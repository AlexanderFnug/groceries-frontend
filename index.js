...
window.addEventListener("load", async () => {
  ...
  const templateUsersModal = await loadHtml(
    "./pages/users-modal/users-modal.html"
  );
  const templateUsersNavigate = await loadHtml(
    "./pages/users-navigate/users.html"
  );
  ...
  const templateProducts = await loadHtml("./pages/addProduct/addproduct.html");
  ...
  router
    ...
    .on({
     ...
     `),
      ...
      "/products": () => {
        renderHtml(templateProducts, "content");
        initAddProduct();
      },
